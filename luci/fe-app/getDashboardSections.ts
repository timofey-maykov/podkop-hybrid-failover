            ...
            .map((item, index) => ({
              ...
              displayName:
                index === 0
                  ...
                  : getProxyUrlName(failoverLinks[index - 1]) ||
                    item?.value?.name ||
                    '',
              latency: item?.value?.history?.[0]?.delay || 0,
              type: item?.value?.type || '',
